...
class MixerExample(Component):
    ...
    def run(self):
        ...
        # Ensure sizes match for blending (Resize img2 to match img1)
        h, w = img1.value.shape[:2]
        img2_resized = cv2.resize(img2.value, (w, h))


        mixed = cv2.addWeighted(img1.value, 1 - self.alpha, img2_resized, self.alpha, 0)

        diff = cv2.absdiff(img1.value, img2_resized)


        self.image_mix = Image.set_frame(img=mixed, package_uID=self.uID, redis_db=self.redis_db)
        self.image_diff = Image.set_frame(img=diff, package_uID=self.uID, redis_db=self.redis_db)
